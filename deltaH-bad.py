...
def remove_existing_combinations(list1, list2):
    set1 = {tuple(combo) for combo in list1}  # Convert to set of tuples for fast lookup
    return [combo for combo in list2 if tuple(combo) not in set1]
...
list_comp = all_combinations(input_list1,ncat)

print(list_comp)
strucname = "baddeleyite"
nsample = 1
...
for i in range(startnum,finnum):
    #Here have put O in twice for the 2 sublattices with O
    basis_elements=[list_comp[i],['O'],['O']]
    print(basis_elements)

    delim=''
    res = delim.join([str(ele) for ele in basis_elements[0]])
    db_name = strucname+"-"+str(ncat)+"comp_"+str(res)+".db"
    print(db_name)
    #Have not included both O separately
    lst = ([1] * ncat) + [2.0]
    A_eq = [lst]
    b_eq = [1.0]
    #Also not inlcuded O separately
    concrange=[ncat*[(0,(1.0/ncat))],[(1.0,1.0)]]
    #print(concrange)
    #Here have put in grouped_basis for O
    conc = Concentration(basis_elements=basis_elements,grouped_basis=[(0,),(1,2)], A_eq=A_eq, b_eq=b_eq)
    conc.set_conc_ranges(ranges=concrange)
    #baddeleyite structure
    from clease.settings import CECrystal
    settings = CECrystal(concentration=conc,
        spacegroup=14,
        basis=[(0.2755, 0.0397, 0.2080), (0.0742, 0.3316, 0.3467),(0.4487, 0.7581, 0.4801)],
        cell=[5.1170, 5.1754, 5.2915, 90, 99.216, 90],
        size=[4,4,4],
        db_name=db_name,
